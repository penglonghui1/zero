...
type SOAPEnvelope struct {
	...
	Soapenv string   `xml:"soapenv,attr,omitempty"`
	Header  *SOAPHeader
	Body    SOAPBody
}
...
type SOAPHeader struct {
	...
	Soap    string        `xml:"soap,attr,omitempty"`
...
func (s *SOAPClient) Call(soapAction string, request SOAPEnvelope, response interface{}) error {
	if s.headers != nil && len(s.headers) > 0 {
		request.Header = &SOAPHeader{Items: s.headers}
	}
...
//NewSOAPServer 新建SOAP服务器
func NewSOAPServer(addr string, handleFunc func([]byte) interface{}) *http.Server {
...
// Scan 扫描参数到结构体
func (s SOAPEnvelope) Scan(rawBody []byte, dest interface{}) error {
	s.Body.Content = dest
