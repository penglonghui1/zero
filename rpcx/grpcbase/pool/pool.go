...
func (p *pool) Get() (Conn, error) {
	...
	if nextRef <= current*int32(p.opt.MaxConcurrentStreams) {
		return p.getActiveConn(current), nil
	}
	...
	if current < int32(p.opt.MaxActive) && nextRef > current*int32(p.opt.MaxConcurrentStreams) {
		...
		err, current := p.createNew(increment, current)
...
func (p *pool) createNew(increment int32, current int32) (error, int32) {
	var i int32
	var err error
...
func (p *pool) getActiveConn(current int32) *conn {
	for {
		if len(p.conns) == 0 {
			_, current = p.createNew(int32(p.opt.MaxIdle), current)
		}
		next := atomic.AddUint32(&p.index, 1) % uint32(current)
		var cconn = p.conns[next]
		if cconn.cc == nil || cconn.cc.GetState() == connectivity.Shutdown {
			var x = make([]*conn, 0)
			x = append(x, p.conns[:p.index]...)
			if int(p.index+1) < len(p.conns) {
				x = append(x, p.conns[p.index+1:]...)
			}
			next = atomic.AddUint32(&p.index, 1) % uint32(current)
			p.conns = x
		} else {
